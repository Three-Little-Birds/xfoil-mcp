...
def compute_polar(request: PolarRequest) -> PolarResponse:
    ...
    if result.returncode != 0:
        raise RuntimeError(
            f"XFOIL execution failed: {result.stderr.decode('utf-8', errors='ignore')}"
        )

    if not polar_path.exists():
        raise RuntimeError("XFOIL did not create the polar output")

    rows = list(csv.reader(polar_path.read_text(encoding="utf-8").splitlines()))
